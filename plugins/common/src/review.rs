//! A code review as one message for the agent (Spec C §4.4, Spec K-7): the
//! body a review page or a GitHub review is mapped to, its limits, and the
//! message `send_text` delivers.

use balerix_api::check_path;
use serde::Deserialize;

/// The most comments one review may carry.
...
/// Which side of the diff a comment's line is on.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Deserialize)]
#[serde(rename_all = "lowercase")]
pub enum Side {
    /// The line as it was before the change.
    Old,
    /// The line as it is after the change.
    New,
}
...
pub struct Comment {
    /// The file the comment was made on, workspace-relative.
    pub path: String,
    /// Which side of the diff `line` counts against.
    pub side: Side,
    /// The line number on `side`.
    pub line: u64,
    /// The quoted diff line, sign included; may be empty.
    #[serde(default)]
    pub text: String,
    /// The comment itself.
    pub body: String,
...
pub struct Review {
    /// The commit reviewed, shown short in the message header.
    pub head: String,
    /// What `head` is reviewed against; empty when there is none.
    pub base_ref: String,
    /// The overall comment; empty when the review is line comments only.
    pub summary: String,
    /// The line comments, in no particular order.
...
pub fn validate(body: &Review) -> Result<(), String> {
...
pub fn render_message(body: &Review) -> String {
...
mod tests {
    ...
    fn the_limits_are_enforced_with_a_field_path() {
        ...
        assert!(
            serde_json::from_value::<Review>(json!({ "comments": [], "nope": 1 })).is_err(),
            "unknown fields are refused"
        );
        let minimal: Review = serde_json::from_value(json!({})).unwrap();
