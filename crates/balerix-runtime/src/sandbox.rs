...
pub fn check_conflicts(
    ...
    if u.contains_key("environment") {
        return Err(conflict(
            "environment".into(),
            "balerix-owned; set variables through `env` instead".into(),
        ));
    }
            ...
            for (g, balerix_level) in hits {
                if overlaps(&p, g) && balerix_level != user_level {
                    return Err(conflict(
                        ...
                        format!(
                            "{} overlaps balerix {balerix_level} path {}",
                            p.display(),
                            g.display()
                        ),
                    ));
...
mod tests {
    ...
    fn base_profile_has_the_required_grants_env_and_port() {
        ...
        assert_eq!(
            p["filesystem"]["read"][5], "/h/.local/state/balerix/fleets/f/crews/c/mise",
            "crew pool"
        );
        assert_eq!(
            p["filesystem"]["read"][6], "/h/.local/state/balerix/fleets/f/mise",
            "fleet pool"
        );
        assert_eq!(
            p["filesystem"]["read"][7], "/h/.local/share/balerix/mise",
            "daemon pool"
        );
        assert_eq!(
            p["filesystem"]["read"][8],
            "/h/.local/state/balerix/fleets/f/crews/c/agents/a/mise.toml"
        );
        assert_eq!(p["filesystem"]["read"][9], "/opt/balerix");
        assert_eq!(
    ...
    fn owned_keys_and_overlapping_grants_are_rejected_with_paths() {
        ...
        assert_eq!(
            e.to_string(),
            "f/c/a: sandbox.environment: balerix-owned; set variables through `env` instead"
        );
        let e = render_profile(
            &id,
            &grants,
            1,
            &env,
            &json!({ "filesystem": { "read": ["/x", "/h/.local/state/balerix/fleets/f/crews/c/agents/a/home/.claude"] } }),
        )
        .unwrap_err();
        assert_eq!(
            e.to_string(),
